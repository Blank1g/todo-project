...
> Блокування виконання коду потрібно уникати усіма шляхами, так як Node.js є однопоточним, тому в ньому використовуються event loop та worker, для того щоб опрацьовувати асинхронні задачі, або виділяти під задачі окремі воркери, які будуть їх виконувати. <br />

> Worker Pool використовується для інтенсивних задач по типу  <br />
> I/O-intensive, якими є  <br />
...
> Worker threads в Node.js дозволяють виконувати JavaScript в паралельних потоках. Кожен воркер працює зі своїм власним інстансом V8 і своїм подієвим циклом.  <br />

> Child processes дозволяють створювати нові процеси, які виконують Node.js або інші програми.  <br />

> Використання Worker threads є зазвичай більш ефективним для JS-задач, а child processes - для виконання більш складних або ресурсомістких операцій на рівні системи або для запуску інших програм.   <br />

76. https://nodejs.org/en/learn/asynchronous-work/event-loop-timers-and-nexttick <br />

> ![image](https://github.com/user-attachments/assets/c5dcae7a-9b20-43f9-a978-cbf5fe60305f)  <br />

> Timers Phase: У цій фазі виконуються колбеки, заплановані через setTimeout() або setInterval(). Основний підводний камінь тут полягає в тому, що час вказує коли колбек буде доданий до черги, а не гарантований час його виконання. Великі блокування потоку в попередніх етапах можуть затримати виконання таймерів.  <br />

> I/O Callbacks Phase: Ця фаза присвячена переважно обробці колбеків за винятком колбеків певних таймерів, close events, setImmediate() і інші. Одна з проблем, з якою можна зіткнутися в цій фазі - це велика кількість вхідних I/O операцій, яка може призвести до I/O голодування, де колбеки для обробки I/O заповняють петлю і відкладають виконання інших типів колбеків.  <br />

> Poll Phase: У цій фазі Node.js чекає на нові I/O події та виконує їх колбеки, як тільки вони стають доступними. Важливо відзначити, що Node.js може застрягти в цій фазі, якщо постійно надходять нові I/O події, що знову може призвести до I/O голодування.  <br />

> Check Phase: Ця фаза наступає тільки якщо у черзі є колбек setImmediate(). SetImmediate() є способом викликати колбек після завершення виконання поточної фази I/O, на відміну від process.nextTick що запускає колбек негайно після поточної операції.  <br />

> Close Callbacks Phase: У цій фазі виконуються колбеки, пов'язані з закриванням веб-сокетів чи інших з'єднань. Типові підводні камені тут пов'язані з необхідністю очищення ресурсів і можливими затримками в зв'язку з очікуванням на завершення всіх обробників.  <br />

> process.nextTick(): Хоча це не фаза петлі подій, але process.nextTick() має істотний вплив на поведінку петлі подій. Функції process.nextTick() завжди виконуються до переходу до наступної фази петлі. Важливо не зловживати цим, оскільки вони можуть збільшити час виконання петлі та затримати інші колбеки.  <br />

> process.nextTick() <br />
> Як і Де викоирстовувати: <br />
>   - Корекція порядку виклику: Наприклад, якщо ви маєте подію, яка повинна відбутись після повної ініціалізації об'єкта, але код, який ви використовуєте, ще не завершений в даний момент. <br />
>   - Події, які повинні відбутися якомога раніше, але після виконання всього синхронного коду. <br />
>   - Для обходу потенційних помилок, зв'язаних з рекурсивністю або максимальною глибиною стеку викликів, наприклад, коли функція викликає сама себе. <br />
> Коли не використовувати: <br />
>   - Не використовуйте для коду, що може створити надмірне навантаження: Багаторазове використання process.nextTick() може затримати виконання іншої асинхронної операції, такої як I/O, створюючи затримки у роботі програми. <br />
>   - Не підходить для того, щоб замінити сет таймери: Якщо потреба відкласти виконання дії на певний час, краще використати setTimeout() або setInterval(). <br />
